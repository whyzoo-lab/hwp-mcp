...
/// haystack 에 `<tag...` 형태의 태그가 있는지 대소문자 무시로 검사한다(접두 일치 —
/// 기존 `lower.contains("<p")` 의미 유지). 데이터 URI 가 포함된 수 MB 콘텐츠에서
/// `to_lowercase()` 전체 복사를 피하기 위한 무할당 스캔: '<' 위치에서만 비교한다.
/// tags 는 '<' 없이 소문자로 준다(예: `&["p", "table", "img"]`).
pub(crate) fn contains_tag_ci(haystack: &str, tags: &[&str]) -> bool {
    ...
    while i < hay.len() {
        if hay[i] == b'<' {
            for t in tags {
                let tb = t.as_bytes();
                if i + 1 + tb.len() <= hay.len()
                    && hay[i + 1..i + 1 + tb.len()].eq_ignore_ascii_case(tb)
                {
                    return true;
                }
            }
...
/// HWP 테두리 선 굵기 인덱스 → 밀리미터 표(스펙 표 "선 굵기").
/// 인덱스 0~15 전체. 예전 매핑은 인덱스 7(0.5mm)에서 잘려 1pt 이상 굵은 선이 모두
/// 얇아졌다(2pt→0.5mm 클램프). 전체 표에 대한 최근접 매칭으로 굵은 선까지 재현한다.
const HWP_BORDER_WIDTHS_MM: [f64; 16] = [
    0.1, 0.12, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.7, 1.0, 1.5, 2.0, 3.0, 4.0, 5.0,
];

/// CSS border 두께(pt)를 HWP 선 굵기 인덱스(0~15)로 변환한다.
/// mm 로 환산한 뒤 HWP 굵기 표에서 가장 가까운 값을 고른다(클램프 없음).
pub(crate) fn css_border_width_to_hwp(pt: f64) -> u8 {
    let mm = pt * 0.352_777_8; // 1pt = 25.4/72 mm ≈ 0.35278mm
    let mut best = 0usize;
    let mut best_diff = f64::MAX;
    for (i, &w) in HWP_BORDER_WIDTHS_MM.iter().enumerate() {
        let d = (mm - w).abs();
        if d < best_diff {
            best_diff = d;
            best = i;
...
mod tests {
    ...
    #[test]
    fn css_border_width_covers_full_range_no_clamp() {
        // 얇은 선: 최근접 매칭
        assert_eq!(css_border_width_to_hwp(0.5), 3); // 0.176mm → 0.2mm(#3)
        assert_eq!(css_border_width_to_hwp(1.0), 6); // 0.353mm → 0.4mm(#6)
        // 굵은 선: 예전엔 0.5mm(#7)로 클램프됐던 것들
        assert_eq!(css_border_width_to_hwp(2.0), 9); // 0.706mm → 0.7mm(#9)
        assert_eq!(css_border_width_to_hwp(3.0), 10); // 1.06mm → 1.0mm(#10)
        assert_eq!(css_border_width_to_hwp(6.0), 12); // 2.12mm → 2.0mm(#12)
        assert_eq!(css_border_width_to_hwp(14.0), 15); // 4.94mm → 5.0mm(#15)
    }
