...
impl DocumentCore {
    /// 문단 스타일 목록을 반환한다. `[{"index","name","type"}, ...]`
    pub fn list_styles_native(&self) -> String {
        let items: Vec<String> = self
            ...
            .enumerate()
            .map(|(i, s)| {
                let name = if !s.local_name.is_empty() {
                    &s.local_name
                } else {
                    &s.english_name
                };
    ...
    /// 제목 문단(굵게 + 큰 글자)을 개요로 추출한다. `[{"level","text","section","para"}, ...]`
    /// 판정: 문단 첫 글자모양이 굵고 base_size >= 1300(13pt). 레벨은 크기로 1~3.
    pub fn get_document_outline_native(&self) -> String {
        ...
        for (si, section) in self.document.sections.iter().enumerate() {
            for (pi, para) in section.paragraphs.iter().enumerate() {
                ...
                // 제목 판정: 문단 내 '굵은' 글자모양 중 가장 큰 크기를 본다.
                // (첫 문단은 기본 ref + 제목 ref가 섞여 위치 기반 판정이 불안정하므로
                //  문단 단위로 최대 굵은 크기를 취한다.) 짧은 텍스트만 제목 후보.
                let max_bold_size = para
                    .char_shapes
                    .iter()
                    .filter_map(|r| self.document.doc_info.char_shapes.get(r.char_shape_id as usize))
                    .filter(|cs| cs.bold)
                    .map(|cs| cs.base_size)
                    .max()
                    .unwrap_or(0);
                if max_bold_size >= 1300 && para.text.chars().count() < 80 {
                    let level = if max_bold_size >= 1700 {
                        1
                    } else if max_bold_size >= 1400 {
                        2
                    } else {
                        3
                    };
                    items.push(format!(
                        "{{\"level\":{},\"text\":{},\"section\":{},\"para\":{}}}",
                        level,
                        json_escape(para.text.trim()),
                        si,
                        pi
                    ));
                }
    ...
    pub fn add_bookmark_native(
        ...
        // HWP에서 로드된 문서는 섹션에 raw_stream이 캐시돼 있어, 이를 비우지 않으면
        // 직렬화기가 북마크 추가 전 옛 바이트를 그대로 써 북마크가 유실된다.
    ...
    pub fn rename_bookmark_native(
        ...
        let renamed = if let Control::Bookmark(ref mut bm) = paragraph.controls[ctrl_idx] {
        ...
        if renamed {
            // 캐시된 raw_stream을 비워 변경된 이름이 직렬화에 반영되게 한다.
            self.document.sections[sec].raw_stream = None;
            Ok(r#"{"ok":true}"#.to_string())
        } else {
